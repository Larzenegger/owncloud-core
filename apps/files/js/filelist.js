...
(function() {
	...
	FileList.prototype = {
		 ...
		 * @return {Promise} promise that resolves with the file info, or null if
		 * no info was fetched, or fails if the file was not found.
		 */
		...
		setupUploadEvents: function($uploadEl) {
			...
			$uploadEl.on('fileuploaddone', function(e, data) {
				...
				var fileName = data.upload.getFileName();
				var fetchInfoPromise = self.addAndFetchFileInfo(OC.basename(fileName), OC.dirname(data.upload.getFullPath() + '/'), {replace: true});
				if (!self._uploads) {
					self._uploads = {};
				}
				if (OC.isSamePath(OC.dirname(data.upload.getFullPath() + '/'), self.getCurrentDirectory())) {
